...
@endif
        <h1 class="mb-4">Creat Activities</h1>
        <form method="post" action="{{ route('activities.insert') }}">
            @csrf
            <div class="form-group">
                <label for="activity_name">Name</label>
                <input type="text" name="activity_name" class="form-control @error('activity_name') is-invalid @enderror " id="activity_name" value=" {{ old('activity_name') }} ">
            ...
            <div class="form-group">
                ...
                <select name="section_id" id="section_id" class="form-control @error('section_id') is-invalid @enderror">
                    ...
                    @foreach (App\Section::all() as $sec)
                        <option value="{{ $sec->id }}" @if(old('section_id') == $sec->id) selected @endif >{{ $sec->section_name }}</option>
                    @endforeach
                </select>
                @error('section_id')
            ...
            <div class="form-group">
                ...
                <input type="text" name="teacher" class="form-control @error('teacher') is-invalid @enderror " id="teacher" value=" {{ old('teacher') }} ">
            ...
            <div class="form-group">
                ...
                <textarea name="requirments" class="form-control @error('requirments') is-invalid @enderror " id="requirments"> {{ old('requirments') }} </textarea>
                @error('requirments')
                <p class="text-danger"> {{ $message }} </p>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="max_students" class="form-control @error('max_students') is-invalid @enderror " id="max_students" value=" {{ old('max_students') }} ">
